    ...
    <div class="panel-heading">
        ...
        <table class="table table-bordered table-hover">
            ...
            foreach ($model as $val): 
                                ...
                                <div class="modal fade" role="dialog" aria-hidden="true" id="id<?= $val->idarchivodir ?>">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <?= Html::beginForm(Url::toRoute(["directorio-archivos/editar", 'token' => $token]), "POST") ?>
                ...
                <td style= 'width: 20px; height: 20px;'>
                    <?php if($token == 0){?>
                      <a href="<?= Url::toRoute(["directorio-archivos/borrar", "id" => $val->idarchivodir, 'numero' => $numero, 'codigo' => $codigo,'view' => $view, 'token' => $token]) ?>" ><span class="glyphicon glyphicon-remove"></span></a>
                    <?php }?>
                </td>
            </tr>
            </tbody>
        ...
        <div class="panel-footer text-right" >
            <?php if($token == 0){?>
                ...
                <?= Html::a('<span class="glyphicon glyphicon-upload"></span> Subir Archivo', ['directorio-archivos/subir','numero' => $numero, 'codigo' => $codigo,'view' =>$view, 'token' => $token], ['class' => 'btn btn-success btn-sm']); ?>            
            <?php }else{?>
               <?= Html::a('<span class="glyphicon glyphicon-circle-arrow-left"></span> Regresar', [$view.'/view', 'id' => $codigo, 'token' => $token], ['class' => 'btn btn-primary btn-sm']); ?>
            <?php }?>
        </div>
