...
$this->title = 'Detalle';
$this->params['breadcrumbs'][] = ['label' => 'Solicitud de compras', 'url' => ['index']];
$this->params['breadcrumbs'][] = $model->id_solicitud_compra;
...
<div class="solicitud-compra-view">
    ...
    <p>
        ...
        <?php }else{?>
            <?= Html::a('<span class="glyphicon glyphicon-circle-arrow-left"></span> Regresar', ['search_consulta_solicitud_compra'], ['class' => 'btn btn-primary btn-sm']) ?>

        <?php }?>
            <?php if ($model->autorizado == 0 && $model->numero_solicitud == 0) { ?>
                <?= Html::a('<span class="glyphicon glyphicon-pencil"></span> Editar', ['update', 'id' => $model->id_solicitud_compra], ['class' => 'btn btn-success btn-sm']) ?>
                <?= Html::a('<span class="glyphicon glyphicon-ok"></span> Autorizar', ['autorizado', 'id' => $model->id_solicitud_compra, 'token' => $token], ['class' => 'btn btn-default btn-sm']);
            } else {
                if ($model->autorizado == 1 && $model->numero_solicitud == 0){
                    echo Html::a('<span class="glyphicon glyphicon-remove"></span> Desautorizar', ['autorizado', 'id' => $model->id_solicitud_compra, 'token' => $token], ['class' => 'btn btn-default btn-sm']);
                      echo Html::a('<span class="glyphicon glyphicon-remove"></span> Cerrar solicitud', ['cerrarsolicitud', 'id' => $model->id_solicitud_compra, 'token'=> $token],['class' => 'btn btn-warning btn-sm',
                               'data' => ['confirm' => 'Esta seguro de cerrar la solicitud de compra.', 'method' => 'post']]);
                    echo Html::a('<span class="glyphicon glyphicon-print"></span> Imprimir', ['imprimirsolicitud', 'id' => $model->id_solicitud_compra], ['class' => 'btn btn-default btn-sm']);            
                }else{
                    echo Html::a('<span class="glyphicon glyphicon-print"></span> Imprimir', ['imprimirsolicitud', 'id' => $model->id_solicitud_compra], ['class' => 'btn btn-default btn-sm']);            
                   echo Html::a('<span class="glyphicon glyphicon-folder-open"></span> Archivos', ['directorio-archivos/index','numero' => 4, 'codigo' => $model->id_solicitud_compra,'view' => $view, 'token' => $token,], ['class' => 'btn btn-default btn-sm']);
                }
            }?>        
    </p>  
                                    ...
                                    <body>
                                         ...
                                         foreach ($detalle_compras as $val):?>
                                            <tr style="font-size: 90%;">
                                                ...
                                                <td style="text-align: right"><?= ''.number_format($val->total_solicitud,0) ?></td>
                                                <input type="hidden" name="detalle_compra[]" value="<?= $val->id ?>">
                                                <td style= 'width: 25px; height: 25px;'>
                                                        <?php 
                                                        if($model->autorizado == 0){?>
                                                           <?= Html::a('<span class="glyphicon glyphicon-trash"></span> ', ['eliminar', 'id' => $model->id_solicitud_compra, 'detalle' => $val->id, 'token' => $token], [
                                                                      'class' => '',
                                                                      'data' => [
