    ...
    <p>
        ...
        <?php }else{
            if($model->cerrar_proceso == 0){?>
                ...
                <button type="button" class="btn btn-default btn-sm"><?= Html::a('<span class="glyphicon glyphicon-folder-close"></span> Cerrar pago', ['close_cast', 'id' => $model->id_pago_banco, 'tipo_proceso' => $model->id_tipo_nomina, 'token' => $token], ['class' => 'btn btn-default btn-sm',
           ...
           <?php }else{?>
                <div class="btn-group" role="group" aria-label="...">
                    ...
                    <button type="button" class="btn btn-default btn-sm"> <?= Html::a('<span class="glyphicon glyphicon-folder-close"></span> Cerrar pago', ['close_cast', 'id' => $model->id_pago_banco, 'token' => $token], ['class' => 'btn btn-default btn-sm disabled',
                        'data' => ['confirm' => 'Esta seguro que desea cerrar el pago a banco No '. $model->id_pago_banco. '', 'method' => 'post']])?></button>
                    ...
                    <button type="button" class="btn btn-default btn-sm"> <?=  Html::a('<span class="glyphicon glyphicon-folder-open"></span> Archivos', ['directorio-archivos/index','numero' => 25, 'codigo' => $model->id_pago_banco,'view' => $view, 'token' => $token,], ['class' => 'btn btn-default btn-sm'])?></button>
                    ...
                    <div class="btn-group" role="group">
                         <button type="button" class="btn btn-info btn-lg dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                           ...
                           <ul class="dropdown-menu">
                            <li><?= Html::a('<span class="glyphicon glyphicon-save-file"></span> Archivo TXT', ['pagoarchivopab', 'id' => $model->id_pago_banco]) ?></li>
                           </ul>
                     </div>
                </div>    
            <?php }    
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped table-hover">
                  <tr style="font-size: 85%;">
                    ...
                    <td><?= Html::encode($model->codigoBanco->entidad_bancaria) ?></td>
                                        ...
                                        <tr style='font-size:85%;'>
                                           <?php 
                                            if($listados->tipo_pago == 1 || $listados->tipo_pago == 2 || $listados->tipo_pago == 3){
                                                $empleado = \app\models\Empleados::find()->where(['=','nit_cedula', $listados->documento])
                                                ...
                                                <td><?= $empleado->transacion?></td>
                                                <td><?= $listados->codigo_banco ?></td>
                                                <td><?= $empleado->banco->entidad ?></td>
                                                        ...
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    ...
                                                                    <h4 class="modal-title">Eliminar registro (DIAMANTE ERP)</h4>
                                                                </div>
