...
class PDF extends FPDF {
     function Header() {
        ...
        $factura = FacturaVentaPunto::findOne($id_factura);
     ...
     function EncabezadoDetalles() {
        ...
        $w = array(16, 55, 12,  15, 17, 14, 18, 20, 25);
    ...
    function Body($pdf,$model) {
        $config = MatriculaEmpresa::findOne(1);
        $detalles = FacturaVentaPuntoDetalle::find()->where(['=','id_factura',$model->id_factura])->all();
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 8);
        $cant = 0;
        foreach ($detalles as $detalle) { 
            ...
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(55, 4, $detalle->producto , 0, 0, 'L');
       ...
       //FIN
        ...
        //fin
        $pdf->SetXY(157, 220);
        $pdf->MultiCell(20, 8, 'RETENCION:',1,'L');
        $pdf->SetXY(177, 220);
...
$pdf->Output("Factura_venta_Punto_$model->numero_factura.pdf", 'D');
exit;
