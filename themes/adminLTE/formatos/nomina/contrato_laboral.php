...
class PDF extends FPDF {
    ...
    function Header() {
        if ( $this->PageNo() == 1 ) {
            ...
            $contrato = Contratos::findOne($id_contrato);
            $config = MatriculaEmpresa::findOne(1);
            $municipio = Municipios::findOne($config->codigoMunicipio->municipio);
            $departamento = Departamentos::findOne($config->codigoDepartamento->departamento);
            //Logo
            $this->SetXY(53, 10);
            $this->Image('dist/images/logos/logoempresa.png', 10, 10, 30, 26);
            //Encabezado
        ...
        $this->Cell(30, 5, utf8_decode("Tipo regimen:"), 0, 0, 'l', 1);
         $this->SetFont('Arial', '', 8);
        $this->Cell(60, 5, utf8_decode($config->tipoRegimen->regimen), 0, 0, 'L', 1);
        $this->SetXY(40, 5);
            //$this->SetXY(10, 42);
    ...
    function Body($pdf, $model) {
        $config = Matriculaempresa::findOne(1);
        $contrato = Contratos::findOne($model->id_contrato);
        ...
        $sustitucion1 = $contrato->nit_cedula;
        $sustitucion2 = utf8_decode($contrato->empleado->nombre_completo);
        $sustitucion3 = $contrato->empleado->codigoMunicipioExpedicion->municipio .' - '. $contrato->empleado->codigoMunicipioExpedicion->codigoDepartamento->departamento;        
        $sustitucion4 = utf8_decode($contrato->empleado->direccion);
        $sustitucion5 = utf8_decode($contrato->empleado->barrio);
        $sustitucion6 = utf8_decode($contrato->cargo->nombre_cargo);
        ...
        $sustitucionc = $contrato->codigoMunicipioContratado->municipio .' - '. $contrato->codigoMunicipioContratado->codigoDepartamento->departamento;    
