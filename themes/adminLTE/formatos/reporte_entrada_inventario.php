...
use app\models\EntradaProductoInventarioDetalle;
...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $orden = EntradaProductosInventario::findOne($id_entrada);
...
//Logo
        ...
        $this->Cell(162, 7, utf8_decode("ENTRADA DE INVENTARIOS"), 0, 0, 'l', 0);
       ...
       // $this->SetFillColor(300, 300, 300);
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(24, 5, utf8_decode("TIPO ORDEN:"), 0, 0, 'L');
        if($orden->id_orden_compra == null){
            $this->SetFont('Arial', '', 8);
            $this->Cell(38, 5, utf8_decode('NO FOUND'), 0, 0, 'L');
        }else{
            $this->SetFont('Arial', '', 8);
            $this->Cell(40, 5, utf8_decode($orden->ordenCompra->tipoOrden->descripcion_orden), 0, 0, 'L');
        }
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("SUBTOTAL:"), 0, 0, 'R');
        $this->SetFont('Arial', '', 8);
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(24, 5, utf8_decode("TIPO ENTRADA:"), 0, 0, 'L');
        $this->SetFont('Arial', '', 8);
        $this->Cell(38, 5, utf8_decode($orden->tipoEntrada), 0, 0, 'L');
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("IMPUESTO:"), 0, 0, 'R');
        $this->SetFont('Arial', '', 8);
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(24, 5, utf8_decode("No SOPORTE:"), 0, 0, 'L');
        $this->SetFont('Arial', '', 8);
        $this->Cell(38, 5, utf8_decode($orden->numero_soporte), 0, 0, 'L');
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(20, 5, utf8_decode("TOTAL:"), 0, 0, 'R');
        $this->SetFont('Arial', '', 8);
    ...
    function Body($pdf, $model) {
        $detalles = EntradaProductoInventarioDetalle::find()->where(['=', 'id_entrada', $model->id_entrada])->all();
        $pdf->SetX(10);
        $pdf->SetFont('Arial', '', 8);
        $i = 0;
