...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $recibo = ReciboCajaPuntoVenta::findOne($id_recibo);
      ...
      //  $this->SetFillColor(200, 200, 200);
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Fecha Pago:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);
        $this->Cell(60, 5, utf8_decode($recibo->fecha_recibo), 0, 0, 'L','0');
        //fin
        $this->SetXY(8, 72); //FILA 2
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Tipo Recibo:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);        
        $this->Cell(60, 5, utf8_decode($recibo->tipo->concepto), 0, 0, 'L','0'); 
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Nro transación:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);        
        $this->Cell(60, 5, utf8_decode($recibo->numero_transacion), 0, 0, 'L','0');                
        //fin
        $this->SetXY(8, 87); //FILA 3
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Valor Pagado:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);
        $this->Cell(60, 5, utf8_decode('$ '.number_format($recibo->valor_abono)), 0, 0, 'L','0');
        //fin
        $this->SetXY(8, 92); //FILA 5
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Punto venta:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);        
        $this->Cell(60, 5, utf8_decode($recibo->punto->nombre_punto), 0, 0, 'L','0');                
        //fin
        $this->SetXY(8, 97); //FILA 3
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Banco:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);
        $this->Cell(60, 5, utf8_decode($recibo->codigoBanco->entidad_bancaria), 0, 0, 'L','0');
        //fin
        $this->SetXY(8, 102); //FILA 7
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(25, 5, utf8_decode("Forma pago:"), 0, 0, 'L','0');
        $this->SetFont('Arial', '', 8);
        $this->Cell(60, 5, utf8_decode($recibo->formaPago->concepto), 0, 0,'L','0');
        //fin
        $this->Line(7,50,95,50);//linea superior horizontal
        $this->Line(7,142,7,50);//primera linea en y
        $this->Line(95,142,95,50);//primera linea en y
        $this->Line(7,142,95,142);//linea inferior horizontal
    ...
    function Body($pdf, $model) {
        ...
        $this->Text(25, 120, utf8_decode('Muchas gracias por su compra!'));
        $this->Text(25, 126, utf8_decode('Fue un placer poderle servir!'));
    }
    function Footer() {

        $this->SetFont('Arial', '', 8);
        $this->Text(8, 138, utf8_decode('Nuestra compañía, en favor del medio ambiente.'));
...
$pdf->Output("Recibo_Caja_No:$model->numero_recibo.pdf", 'D');
