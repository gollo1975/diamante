...
$this->title = 'TRASLADO DE REFERENCIA (' .$model->nombre_producto. ')';
$this->params['breadcrumbs'][] = ['label' => 'Importar unidades', 'url' => ['index']];
$this->params['breadcrumbs'][] = $model->id_inventario;
$conPunto = ArrayHelper::map(\app\models\PuntoVenta::find()->where(['<>','id_punto', 1])->all(), 'id_punto', 'nombre_punto');
?>
                ...
                <div class="table-responsive">
                    <div class="panel panel-success">
                                ...
                                <tbody>
                                    ...
                                    foreach ($talla_color as $tallas):?>
                                        <tr style='font-size:90%;'>
                                            ...
                                            <td><?= Html::dropDownList('id_punto_saliente[]', $tallas->id_punto, $conPunto, ['class' => 'col-sm-10', 'prompt' => 'Seleccione el local']) ?></td>
                                            <td style="padding-right: 1;padding-right: 1; text-align: right"> <input type="text" name="cantidades[]" value="" style="text-align: right" size="9"> </td> 
                                             <input type="hidden" name="nuevo_envio_bodega[]" value="<?= $tallas->id_detalle?>"> 
                                        </tr>
                                    <?php endforeach;?>
                                </tbody>
                            </table>
                        ...
                        <div class="panel-footer text-right">
                            <?= Html::submitButton("<span class='glyphicon glyphicon-send'></span> Enviar traslado", ["class" => "btn btn-primary btn-sm", 'name' => 'enviar_referencia']) ?>
                        </div>
                    </div>
                </div>
            </div>
