...
class PagoBancoController extends Controller
{
   ...
   public function actionIndex($token = 0) {
                ...
                if ($form->load(Yii::$app->request->get())) {
                    if ($form->validate()) {
                        ...
                        $table = PagoBanco::find()
                                ->andFilterWhere(['=', 'codigo_banco', $id_banco])
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            $model->id_empresa = 1;
            $model->nit_cedula = $empresa->nit_empresa;
            $model->save();
            return $this->redirect(['view', 'id' => $model->id_pago_banco, 'token' => 0]);
        }
    ...
    /**
     * Deletes an existing PagoBanco model.
   ...
   //ELIMINAR DETALLE DE PAGO
                ...
                if ($form->validate()) {
                    ...
                    if ($nombres){
                        $listadoPago = \app\models\ProgramacionNomina::find()
                                ->andFilterWhere(['=','id_empleado', $nombres])
        ...
        if (isset($_POST["aplicar_pago"])) {
            ...
            foreach ($_POST["aplicar_pago"] as $intCodigo) {
                ...
                if($tipo_proceso == 1 || $tipo_proceso ==  2 || $tipo_proceso == 3){ //nominas, cesantias y primas
                    ...
                    $empleado = \app\models\Empleados::findOne($nomina->id_empleado);
                    ...
                    if ($reg == 0) {
                        ...
                        $table->concepto_documento = $empleado->tipoDocumento->tipo_documento;
        ...
        if($model->autorizado == 0){
            ...
            if (count($pago)> 0) {
                ...
                $this->redirect(["pago-banco/view", 'id' => $id, 'token' => $token]);
                    ...
                    Yii::$app->getSession()->setFlash('error', 'Para autorizar el registro debe de programar el listado de pagos de nómina.');
                    $this->redirect(["pago-banco/view", 'id' => $id, 'token' => $token]);
            }
        } else {
                $model->autorizado = 0;
     ...
     foreach ($detalle as $detalles):
        if($tipo_proceso == 1 || $tipo_proceso == 2 || $tipo_proceso == 3){ 
            ...
            if($nomina){
                ...
                Yii::$app->getSession()->setFlash('error', 'No hay registros para descargar en la nomina');
                $this->redirect(["pago-banco/view", 'id' => $id, 'token' => $token]);
            }
        }   
     endforeach;
    ...
    public function actionPagoarchivopab($id) {
        ...
        $fijo = 6;
        $letra = 'S';
        $strArchivo = "archivoPlanoPab_$pago_banco->fecha_creacion.$pago_banco->secuencia".".txt";                
   ...
   public function actionImprimir_reporte($id, $token, $tipo_proceso)
     ...
     * @param integer $id
     * @return PagoBanco the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
