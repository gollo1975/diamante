...
/**
 * PeriodoPagoNominaController implements the CRUD actions for PeriodoPagoNomina model.
 */
class PeriodoPagoNominaController extends Controller
{
    ...
    /**
     * Lists all PeriodoPagoNomina models.
     * @return mixed
     */
      
    //PROCESO QUE GENERA LOS PERIODOS DE PAGO
            ...
            if (UsuarioDetalle::find()->where(['=','codusuario', Yii::$app->user->identity->codusuario])->andWhere(['=','id_permiso',134])->all()){
                ...
                if ($form->load(Yii::$app->request->get())) {
                    if ($form->validate()) {                        
                        ...
                        $table = \app\models\GrupoPago::find()
                        ...
                        if(isset($_POST['crear_periodo_nomina'])){                            
                            ...
                            $this->redirect(["periodo-pago-nomina/index_search"]);
                        }
    ...
    //PROCESOS PRIVADOS QUE EJECUTA LOS PERIODOS DE PAGO
     public function actionCrearPeriodoNomina($id_grupo_pago) {
       ...
       $numero_dias_mes = cal_days_in_month(CAL_GREGORIAN, $mes_inicio, $anio_inicio);
        ...
        $periodo_pago_nomina->user_name = Yii::$app->user->identity->username;
    ...
    public function actionCrearPeriodoCesantia($id_grupo_pago) {
        $periodo_pago_nomina = new PeriodoPagoNomina();
        $grupo_pago = GrupoPago::findOne($id_grupo_pago);
        $periodo_pago = \app\models\PeriodoPago::findOne($grupo_pago->id_periodo_pago);                                         
        $dias = 360;
    ...
    /**
     * Displays a single PeriodoPagoNomina model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new PeriodoPagoNomina model.
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->id_periodo_pago_nomina]);
        }
    ...
    /**
     * Updates an existing PeriodoPagoNomina model.
    ...
    /**
     * Deletes an existing PeriodoPagoNomina model.
    ...
    /**
     * Finds the PeriodoPagoNomina model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PeriodoPagoNomina the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PeriodoPagoNomina::findOne($id)) !== null) {
            return $model;
        }
     ...
     public function actionExcelConsulta($tableexcel) {                
        ...
        foreach ($tableexcel as $val) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('E' . $i, $val->municipios->municipio)                    
