...
class ProcesoDisciplinarioController extends Controller
{
            ...
            if ($model->validate()) {
                ...
                if (isset($_POST["crear_proceso"])) {
                    if($model->cedula <> null){
                        $conempleado = Empleados::find()->where(['=','nit_cedula', $model->cedula])->andWhere(['=','estado', 0])->one(); 
                        if($conempleado){
                   
                           $concontrato = Contratos::find()->where(['=','id_empleado', $conempleado->id_empleado])->andWhere(['=','contrato_activo', 0])->one();
    ...
    public function actionGenerar_consecutivo($id, $token) {
        $model = $this->findModel($id);
        $codigo = \app\models\Consecutivos::findOne(31);
        $model->numero_radicado = $codigo->numero_inicial + 1;
        $model->proceso_cerrado = 1;
        $model->save(false);
        //actualiza
        $codigo->numero_inicial = $model->numero_radicado;
        $codigo->save();
   ...
   //proceso que crea la licencia no remunerada
    protected function CrearLicenciaNoRemunerada($id) {
        ...
        if($concepto){
            ...
            $table->identificacion = $model->empleado->nit_cedula;
